...
let modalCallback = null; // Змінна для зберігання функції, яка буде викликана після закриття модалки
...
function showModal(title, message, callback = null) {
    ...
    modalCallback = callback; // Зберігаємо callback
...
function hideModal() {
    ...
    // Після закриття модалки викликаємо callback, якщо він є
    if (modalCallback) {
        modalCallback();
        modalCallback = null; // Очищаємо callback, щоб уникнути повторного виклику
    }
}

// Обробник закриття модалки (завжди прив'язуємо до кнопки)
modalCloseBtn.addEventListener('click', hideModal);

// Оновлення текстових повідомлень (тепер використовується тільки для загального ігрового повідомлення)
...
function resetGame() {
    ...
    updateGameStatusMessage(`Гравець ${currentPlayerPlacing}, розставте свої кораблі.`);
...
function autoPlaceShipsForCurrentPlayer() {
    ...
    showModal("Авторозстановка", "Кораблі автоматично розставлені.");
...
function handlePlacementClick(event) {
    ...
    if (!nextShipToPlace) {
        showModal("Розміщення кораблів", "Всі кораблі вже розставлені. Будь ласка, підтвердіть.");
        return;
    }
    ...
    if (canPlaceShip(currentBoard, nextShipToPlace.size, row, col, currentPlacementOrientation)) {
        ...
        updateGameStatusMessage(`Розміщено ${nextShipToPlace.name}.`);
        updatePlacementUI();
    } else {
        showModal("Помилка розміщення", "Не можна розмістити корабель тут. Занадто близько до іншого корабля або виходить за межі.");
    }
}
...
function handleConfirmPlacement() {
    ...
    if (!allShipsPlaced) {
        showModal("Помилка підтвердження", "Будь ласка, розставте всі кораблі перед підтвердженням.");
        return;
    }

    if (currentPlayerPlacing === 1) {
        currentPlayerPlacing = 2; // Перехід до розстановки Гравця 2
        showModal("Розстановка завершена", "Гравець 1 розставив кораблі. Тепер хід Гравця 2!", updatePlacementUI);
    } else {
        gamePhase = 'game'; // Перехід до ігрової фази
        document.getElementById('placement-section').classList.add('hidden');
        document.getElementById('game-section').classList.remove('hidden');
        showModal("Гра починається!", "Обидва гравці розставили кораблі! Гравець 1, стріляйте по дошці Гравця 2.", () => {
            currentPlayerTurn = 1;
...
function handleShotAttempt(event) {
    ...
    if (currentPlayerTurn === 1 && targetGridElement.id === 'player2-grid') {
        ...
        showModal("Невірний хід", "Клікніть на дошку опонента!");
        return;
    }
    ...
    if ([CELL_HIT, CELL_MISS, CELL_SUNK].includes(targetBoard[row][col])) {
        showModal("Невірний хід", "Ви вже стріляли сюди!");
        return;
    }
    ...
    updateGameStatusMessage(`Гравець ${currentPlayerTurn}: ${message}`);
    ...
    if (checkAllShipsSunk(targetShips)) {
        gameEnded = true;
        showModal("Гра завершена!", `Гравець ${currentPlayerTurn} переміг! Кінець гри.`);
        document.getElementById('player1-grid').classList.remove('active-target-board');
        document.getElementById('player2-grid').classList.remove('active-target-board');
        return;
    }
    ...
    if (!isHit && !gameEnded) {
        ...
        showModal(`Хід Гравця ${newCurrentPlayerId}!`, `Передайте керування Гравцю ${newCurrentPlayerId}! Натисніть ОК, щоб продовжити.`, () => {
            updateGameStatusMessage(`Гравець ${newCurrentPlayerId}, ваш хід! Стріляйте по дошці опонента.`);
            renderAllBoards();
        });
    } else if (isHit && !gameEnded) {
        // Якщо влучання, поточний гравець стріляє знову
        updateGameStatusMessage(`Гравець ${currentPlayerTurn}: ${message} Стріляйте знову!`);
    }
}
